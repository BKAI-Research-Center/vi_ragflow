...
class LayoutRecognizer(Recognizer):
    ...
    def __call__(self, image_list, ocr_res, scale_factor=3,
        ...
        layouts = []
        for layout_res in surya_layouts:
            _page_layout = []
            for layout_box in layout_res.bboxes:
                _type = layout_box.label.lower()
                _bbox = layout_box.bbox
                _score = layout_box.confidence
                _page_layout.append({
                    "type": _type,
                    "bbox": _bbox,
                    "score": _score
                })
            layouts.append(_page_layout)
        ...
        for pn, lts in enumerate(layouts):
            ...
            for lt in [label.lower() for label in self.labels]:
                findLayout(lt)
